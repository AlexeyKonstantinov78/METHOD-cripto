...
const socet = new WebSocket('wss://shy-vivid-halibut.glitch.me/');
...
const renderExchange = (wrapper, data) => {
  ...
  if (wrapper.children.length > 4) wrapper.children[4].remove();
};

socet.addEventListener('message', event => {  
  renderExchange(exchangeRatesList, event.data);
});

socet.addEventListener('error', err => {
  console.log(err);
});